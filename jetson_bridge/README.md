...
- `ARM_RELAX`, `ARM_BUTTON`, `ARM_PRESS`, and `ARM_GRASP_BOTTLE` publish
  fixed action JSON to `/current_arm_subtask`.
...
The four arm buttons send `ARM_RELAX`, `ARM_BUTTON`, `ARM_PRESS`, and
`ARM_GRASP_BOTTLE` through `/command`. The bridge publishes these payloads:
...
`ARM_GRASP_BOTTLE` publishes the exact action name `grasp_water_bottle`;
`move_to_grasp` is not a valid bottle-grasp trigger. `/current_arm_subtask` uses
reliable, transient-local, keep-last depth-1 QoS. Each tap publishes once, so do
not tap repeatedly during an active attempt.

Before dispatch, place the bottle in the wrist-camera view and ensure
